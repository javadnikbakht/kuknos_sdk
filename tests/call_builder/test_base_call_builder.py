...
from kuknos_sdk.__version__ import __version__
from kuknos_sdk.call_builder import BaseCallBuilder
from kuknos_sdk.client.aiohttp_client import AiohttpClient
from kuknos_sdk.client.requests_client import RequestsClient
from kuknos_sdk.exceptions import BadRequestError, NotFoundError, NotPageableError
...
class TestBaseCallBuilder:
    ...
    async def test_get_data_async(self):
        ...
        assert resp["headers"][
            "User-Agent"
        ] == "py-kuknos-sdk/{}/AiohttpClient".format(__version__)
        assert resp["headers"]["X-Client-Name"] == "py-kuknos-sdk"
    ...
    def test_get_data_sync(self):
        ...
        assert resp["headers"][
            "User-Agent"
        ] == "py-kuknos-sdk/{}/RequestsClient".format(__version__)
        assert resp["headers"]["X-Client-Name"] == "py-kuknos-sdk"
        assert resp["headers"]["X-Client-Version"] == __version__
    ...
    async def test_get_stream_data_async(self):
        url = "https://horizon.kuknos.org/ledgers"
        client = AiohttpClient()
    ...
    def test_status_400_raise_sync(self):
        url = "https://horizon.kuknos.org/accounts/BADACCOUNTID"
        client = RequestsClient()
        ...
        assert exception.type == "https://kuknos.org/horizon-errors/bad_request"
    ...
    def test_status_404_raise_sync(self):
        url = "https://horizon.kuknos.org/not_found"
        client = RequestsClient()
        ...
        assert exception.type == "https://kuknos.org/horizon-errors/not_found"
    ...
    def test_get_data_page(self):
        url = "https://horizon.kuknos.org/transactions"
        client = RequestsClient()
        call_builder = (
        ...
        assert first_resp["_links"] == {
            "self": {
                "href": "https://horizon.kuknos.org/transactions?cursor=81058917781504&limit=10&order=desc"
            },
            "next": {
                "href": "https://horizon.kuknos.org/transactions?cursor=12884905984&limit=10&order=desc"
            },
            "prev": {
                "href": "https://horizon.kuknos.org/transactions?cursor=80607946215424&limit=10&order=asc"
            },
        }
        ...
        assert next_resp["_links"] == {
            ...
            "prev": {
                "href": "https://horizon.kuknos.org/transactions?cursor=12884905984&limit=10&order=asc"
            },
        }
        ...
        assert prev_page["_links"] == {
            ...
            "next": {
                "href": "https://horizon.kuknos.org/transactions?cursor=81827716927488&limit=10&order=asc"
            },
            "prev": {
                "href": "https://horizon.kuknos.org/transactions?cursor=33676838572032&limit=10&order=desc"
            },
        }
