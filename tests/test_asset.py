...
from kuknos_sdk.exceptions import AssetIssuerInvalidError, AssetCodeInvalidError
...
class TestAsset:
    ...
    def test_from_xdr_object_native(self):
        xdr_type = kuknos_xdr.AssetType.ASSET_TYPE_NATIVE
        xdr = kuknos_xdr.Asset(type=xdr_type)
    ...
    def test_from_xdr_object_alphanum4(self):
        ...
        issuer = "GCNY5OXYSY4FKHOPT2SPOQZAOEIGXB5LBYW3HVU3OWSTQITS65M5RCNY"
        type = "credit_alphanum4"
        asset_code = kuknos_xdr.AssetCode4(
            bytearray(code, "ascii") + b"\x00" * (4 - len(code))
        )
        asset = kuknos_xdr.AssetAlphaNum4(
            asset_code=asset_code,
            issuer=Keypair.from_public_key(issuer).xdr_account_id(),
        )
        xdr = kuknos_xdr.Asset(
            type=kuknos_xdr.AssetType.ASSET_TYPE_CREDIT_ALPHANUM4, alpha_num4=asset
        )
        asset = Asset.from_xdr_object(xdr)
        assert asset.code == code
        assert asset.issuer == issuer
    ...
    def test_from_xdr_object_alphanum12(self):
        ...
        xdr = kuknos_xdr.Asset(
            type=kuknos_xdr.AssetType.ASSET_TYPE_CREDIT_ALPHANUM12, alpha_num12=asset
        )
        asset = Asset.from_xdr_object(xdr)
        assert asset.code == code
        assert asset.issuer == issuer
