...
from kuknos_sdk.client.aiohttp_client import AiohttpClient, USER_AGENT


class TestAiohttpClient:
    @pytest.mark.asyncio
    async def test_get(self):
        user_agent = "Hello/Kuknos/overcat"
        client = AiohttpClient(pool_size=10, user_agent=user_agent)
        url = "https://httpbin.org/get"
        params = {"hello": "world", "kuknos": "sdk"}
    ...
    async def test_stream(self):
        async with AiohttpClient() as client:
            ...
            async for msg in client.stream(
                "https://horizon.kuknos.org/ledgers", {"cursor": "now"}
            ):
