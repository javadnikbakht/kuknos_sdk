...
from kuknos_sdk.exceptions import NoApproximationError, TypeError
from kuknos_sdk.strkey import StrKey
from kuknos_sdk.utils import (
    best_rational_approximation,
    hex_to_bytes,
    urljoin_with_query,
    parse_ed25519_account_id_from_muxed_account_xdr_object,
)
from kuknos_sdk import xdr as kuknos_xdr
...
class TestUtils:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "https://horizon.kuknos.org/",
                "transaction",
                "https://horizon.kuknos.org/transaction",
            ),
            (
                "https://horizon.kuknos.org/hello",
                "transaction",
                "https://horizon.kuknos.org/hello/transaction",
            ),
            (
                "https://horizon.kuknos.org/?auth=password",
                "transaction",
                "https://horizon.kuknos.org/transaction?auth=password",
            ),
            (
                "https://horizon.kuknos.org/hello?auth=password",
                "transaction",
                "https://horizon.kuknos.org/hello/transaction?auth=password",
            ),
            (
                "https://horizon.kuknos.org?auth=password&name=overcat",
                "transaction",
                "https://horizon.kuknos.org/transaction?auth=password&name=overcat",
            ),
            (
                "https://horizon.kuknos.org/hello?auth=password&name=overcat",
                "transaction",
                "https://horizon.kuknos.org/hello/transaction?auth=password&name=overcat",
            ),
            (
                "https://horizon.kuknos.org/hello/world?auth=password&name=overcat",
                "transaction",
                "https://horizon.kuknos.org/hello/world/transaction?auth=password&name=overcat",
            ),
            (
                "https://horizon.kuknos.org/hello/world?auth=password&name=overcat",
                "",
    ...
    def test_parse_ed25519_account_id_from_muxed_account_xdr_object_ed25519(self):
        ...
        muxed = kuknos_xdr.MuxedAccount(
            type=kuknos_xdr.CryptoKeyType.KEY_TYPE_ED25519,
            ed25519=kuknos_xdr.Uint256(StrKey.decode_ed25519_public_key(account_id)),
        )
        assert (
    ...
    def test_parse_ed25519_account_id_from_muxed_account_xdr_object_muxed_account(self):
        ...
        id = 1234
        med25519 = kuknos_xdr.MuxedAccountMed25519(
            id=kuknos_xdr.Uint64(id), ed25519=kuknos_xdr.Uint256(ed25519)
        )
        muxed = kuknos_xdr.MuxedAccount(
            type=kuknos_xdr.CryptoKeyType.KEY_TYPE_MUXED_ED25519, med25519=med25519
        )
        assert (
            parse_ed25519_account_id_from_muxed_account_xdr_object(muxed) == account_id
        )
