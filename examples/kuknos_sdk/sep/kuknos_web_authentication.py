"""
SEP: 0010
Title: Kuknos Web Authentication
Author: Sergey Nebolsin <@nebolsin>, Tom Quisel <tom.quisel@gmail.com>, Leigh McCulloch <@leighmcculloch>, Jake Urban <jake@kuknos.org>
Status: Active
Created: 2018-07-31
Updated: 2021-03-04
Version 3.2.0
...
def build_challenge_transaction(
    ...
    """Returns a valid `SEP0010 <https://github.com/kuknos/kuknos-protocol/blob/master/ecosystem/sep-0010.md>`_
    challenge transaction which you can use for Kuknos Web Authentication.

    :param server_secret: secret key for server's kuknos.toml `SIGNING_KEY`.
    :param client_account_id: The kuknos account that the wallet wishes to authenticate with the server.
    ...
    :param network_passphrase: The network to connect to for verifying and retrieving
        additional attributes from. (ex. 'Public Global Kuknos Network ; September 2015')
    ...
    :param client_signing_key: The kuknos account listed as the SIGNING_KEY on the client domain's TOML file
...
def read_challenge_transaction(
    ...
    one of the following functions to completely verify the transaction:
        - :func:`kuknos_sdk.sep.kuknos_web_authentication.verify_challenge_transaction_threshold`
        - :func:`kuknos_sdk.sep.kuknos_web_authentication.verify_challenge_transaction_signers`
    ...
    :param network_passphrase: The network to connect to for verifying and retrieving
        additional attributes from. (ex. 'Public Global Kuknos Network ; September 2015')
    :raises: :exc:`InvalidSep10ChallengeError <kuknos_sdk.sep.exceptions.InvalidSep10ChallengeError>` - if the
        validation fails, the exception will be thrown.
    """

    # decode the received input as a base64-urlencoded XDR representation of Kuknos transaction envelope
    ...
    xdr_object = kuknos_xdr.TransactionEnvelope.from_xdr(challenge_transaction)
    if xdr_object.type == kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX_FEE_BUMP:
        raise ValueError(
...
def verify_challenge_transaction_signers(
    ...
    :raises: :exc:`InvalidSep10ChallengeError <kuknos_sdk.sep.exceptions.InvalidSep10ChallengeError>`:
        - The transaction is invalid according to :func:`kuknos_sdk.sep.kuknos_web_authentication.read_challenge_transaction`.
...
def verify_challenge_transaction_signed_by_client_master_key(
    ...
    """An alias for :func:`kuknos_sdk.sep.kuknos_web_authentication.verify_challenge_transaction`.
...
def verify_challenge_transaction(
    ...
    `SEP0010 v1.2 <https://github.com/kuknos/kuknos-protocol/blob/master/ecosystem/sep-0010.md>`_
...
def _verify_transaction_signatures(
    ...
    for signer in signers:
        ...
        for index, decorated_signature in enumerate(transaction_envelope.signatures):
            ...
            # See https://github.com/javadnikbakht/py-kuknos-base/issues/252#issuecomment-580882560
