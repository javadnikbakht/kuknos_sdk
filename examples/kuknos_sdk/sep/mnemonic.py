"""
SEP: 0005
Title: Key Derivation Methods for Kuknos Accounts
"""
...
class KuknosMnemonic(Mnemonic):
    ...
    def derive(seed: bytes, index: int) -> bytes:
        ...
        master_hmac = hmac.new(KuknosMnemonic.SEED_MODIFIER, digestmod=hashlib.sha512)
        master_hmac.update(seed)
        il = master_hmac.digest()[:32]
        ir = master_hmac.digest()[32:]
        path = KuknosMnemonic.STELLAR_ACCOUNT_PATH_FORMAT % index
        for x in path.split("/")[1:]:
            data = (
                struct.pack("x")
                + il
                + struct.pack(">I", KuknosMnemonic.FIRST_HARDENED_INDEX + int(x[:-1]))
            )
