...
from .exceptions import KuknosTomlNotFoundError
...
def fetch_kuknos_toml(
    domain: str,
    client: Union[BaseAsyncClient, BaseSyncClient] = None,
    use_http: bool = False,
) -> Union[Coroutine[Any, Any, Dict[str, Any]], Dict[str, Any]]:
    """Retrieve the kuknos.toml file from a given domain.

    Retrieve the kuknos.toml file for information about interacting with
    Kuknos's federation protocol for a given Kuknos Anchor (specified by a
    domain).
    ...
    :return: The kuknos.toml file as a an object via :func:`toml.loads`.
    :raises: :exc:`KuknosTomlNotFoundError <kuknos_sdk.sep.exceptions.KuknosTomlNotFoundError>`:
        if the Kuknos toml file could not not be found.
    """
    if not client:
        client = RequestsClient()

    toml_link = "/.well-known/kuknos.toml"
    protocol = "https://"
    if use_http:
        protocol = "http://"
