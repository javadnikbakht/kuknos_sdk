...
class SignerKey:
    """The :class:`SignerKey` object, which represents an account signer key on Kuknos's network.

    :param signer_key: The XDR signer object
    """

    def __init__(self, signer_key: kuknos_xdr.SignerKey) -> "None":
        self.signer_key: kuknos_xdr.SignerKey = signer_key
    ...
    def ed25519_public_key(cls, account_id: str) -> "SignerKey":
        ...
        :raises:
            :exc:`Ed25519PublicKeyInvalidError <kuknos_sdk.exceptions.Ed25519PublicKeyInvalidError>`: if ``account_id``
            is not a valid ed25519 public key.
        """
        signer_key = kuknos_xdr.SignerKey(
            kuknos_xdr.SignerKeyType.SIGNER_KEY_TYPE_ED25519,
    ...
    def pre_auth_tx(cls, pre_auth_tx_hash: bytes) -> "SignerKey":
        ...
        click `here <https://www.kuknos.org/developers/guides/concepts/multi-sig.html#pre-authorized-transaction>`__ for more information.

        :param pre_auth_tx_hash: The sha256 hash of a transaction.
        :return: Pre AUTH TX Signer
        """
        signer_key = kuknos_xdr.SignerKey(
            kuknos_xdr.SignerKeyType.SIGNER_KEY_TYPE_PRE_AUTH_TX,
            pre_auth_tx=kuknos_xdr.Uint256(pre_auth_tx_hash),
        )
    ...
    def sha256_hash(cls, sha256_hash: bytes) -> "SignerKey":
        ...
        click `here <https://www.kuknos.org/developers/guides/concepts/multi-sig.html#hashx>`__ for more information.

        :param sha256_hash: a sha256 hash of a preimage
        :return: SHA256 HASH Signer
        """
        signer_key = kuknos_xdr.SignerKey(
            kuknos_xdr.SignerKeyType.SIGNER_KEY_TYPE_HASH_X,
            hash_x=kuknos_xdr.Uint256(sha256_hash),
        )
        return cls(signer_key)

    def to_xdr_object(self) -> kuknos_xdr.SignerKey:
    ...
    @classmethod
    def from_xdr_object(cls, xdr_object: kuknos_xdr.SignerKey) -> "SignerKey":
        ...
        """
        if xdr_object.type == kuknos_xdr.SignerKeyType.SIGNER_KEY_TYPE_ED25519:
        ...
        elif xdr_object.type == kuknos_xdr.SignerKeyType.SIGNER_KEY_TYPE_PRE_AUTH_TX:
            assert xdr_object.pre_auth_tx is not None
            return cls.pre_auth_tx(xdr_object.pre_auth_tx.uint256)
        elif xdr_object.type == kuknos_xdr.SignerKeyType.SIGNER_KEY_TYPE_HASH_X:
