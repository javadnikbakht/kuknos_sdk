...
class ManageData(Operation):
    ...
    ManageData operation on Kuknos's network.
    ...
    _XDR_OPERATION_TYPE: kuknos_xdr.OperationType = kuknos_xdr.OperationType.MANAGE_DATA
    ...
    def _to_operation_body(self) -> kuknos_xdr.OperationBody:
        data_name = kuknos_xdr.String64(bytes(self.data_name, encoding="utf-8"))
        if self.data_value is None:
            data_value = None
        else:
            data_value = kuknos_xdr.DataValue(self.data_value)

        manage_data_op = kuknos_xdr.ManageDataOp(data_name, data_value)
    ...
    @classmethod
    def from_xdr_object(cls, xdr_object: kuknos_xdr.Operation) -> "ManageData":
