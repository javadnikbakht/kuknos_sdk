...
class CreateAccount(Operation):
    ...
    :param starting_balance: Amount in PMN the account should be
        funded for. Must be greater than the `reserve balance amount
        <https://www.kuknos.org/developers/learn/concepts/fees.html>`_.
    :param source: The source account for the payment. Defaults to the
        transaction's source account.

    """

    _XDR_OPERATION_TYPE: kuknos_xdr.OperationType = kuknos_xdr.OperationType.CREATE_ACCOUNT
    ...
    def _to_operation_body(self) -> kuknos_xdr.OperationBody:
        ...
        starting_balance = kuknos_xdr.Int64(
            Operation.to_xdr_amount(self.starting_balance)
        )
        create_account_op = kuknos_xdr.CreateAccountOp(destination, starting_balance)
    ...
    @classmethod
    def from_xdr_object(cls, xdr_object: kuknos_xdr.Operation) -> "CreateAccount":
