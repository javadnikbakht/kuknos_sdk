...
class ClaimPredicate:
    """The :class:`ClaimPredicate` object, which represents a ClaimPredicate on Kuknos's network.
    ...
    **We do not recommend that you build it through the constructor, please use the helper function.**
    ...
    def to_xdr_object(self) -> kuknos_xdr.ClaimPredicate:
        if (
            self.claim_predicate_type
            == ClaimPredicateType.CLAIM_PREDICATE_UNCONDITIONAL
        ):
            return kuknos_xdr.ClaimPredicate(
                kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_UNCONDITIONAL
            )
        elif (
            self.claim_predicate_type
            ...
            return kuknos_xdr.ClaimPredicate(
                kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_BEFORE_ABSOLUTE_TIME,
                abs_before=kuknos_xdr.Int64(self.abs_before),
            )
        elif (
            self.claim_predicate_type
            ...
            return kuknos_xdr.ClaimPredicate(
                kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_BEFORE_RELATIVE_TIME,
                rel_before=kuknos_xdr.Int64(self.rel_before),
            )
        elif self.claim_predicate_type == ClaimPredicateType.CLAIM_PREDICATE_NOT:
            assert self.not_predicate is not None
            return kuknos_xdr.ClaimPredicate(
                kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_NOT,
            ...
            return kuknos_xdr.ClaimPredicate(
                kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_AND,
            ...
            return kuknos_xdr.ClaimPredicate(
                kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_OR,
    ...
    @classmethod
    def from_xdr_object(
        cls, xdr_object: kuknos_xdr.ClaimPredicate
    ) -> "ClaimPredicate":
        ...
        elif claim_predicate_type == kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_NOT:
        ...
        elif claim_predicate_type == kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_AND:
        ...
        elif claim_predicate_type == kuknos_xdr.ClaimPredicateType.CLAIM_PREDICATE_OR:
...
class Claimant:
    ...
    def to_xdr_object(self) -> kuknos_xdr.Claimant:
        claimant_v0 = kuknos_xdr.ClaimantV0(
            destination=Keypair.from_public_key(self.destination).xdr_account_id(),
            predicate=self.predicate.to_xdr_object(),
        )
        claimant = kuknos_xdr.Claimant(
            kuknos_xdr.ClaimantType.CLAIMANT_TYPE_V0, claimant_v0
        )
        return claimant

    @classmethod
    def from_xdr_object(cls, xdr_object: kuknos_xdr.Claimant) -> "Claimant":
...
class CreateClaimableBalance(Operation):
    ...
    <https://developers.kuknos.org/docs/glossary/claimable-balance/>_` for more information on parameters and usage.

    See `Create Claimable Balance
    <https://developers.kuknos.org/docs/start/list-of-operations/#create-claimable-balance>_`.
    ...
    _XDR_OPERATION_TYPE: kuknos_xdr.OperationType = kuknos_xdr.OperationType.CREATE_CLAIMABLE_BALANCE
    ...
    def _to_operation_body(self) -> kuknos_xdr.OperationBody:
        ...
        create_claimable_balance_op = kuknos_xdr.CreateClaimableBalanceOp(
            asset=asset, amount=kuknos_xdr.Int64(amount), claimants=claimants
        )
