...
class AuthorizationFlag(IntFlag):
    ...
    please refer to the `accounts doc <https://www.kuknos.org/developers/guides/concepts/accounts.html>`_.
...
class SetOptions(Operation):
    ...
    doc <https://www.kuknos.org/developers/guides/concepts/multi-sig.html>`_.
    ...
    _XDR_OPERATION_TYPE: kuknos_xdr.OperationType = kuknos_xdr.OperationType.SET_OPTIONS
    ...
    def _to_operation_body(self) -> kuknos_xdr.OperationBody:
        ...
        home_domain = (
            kuknos_xdr.String32(bytes(self.home_domain, encoding="utf-8"))
            if self.home_domain is not None
            else None
        )
        clear_flags = (
            None if self.clear_flags is None else kuknos_xdr.Uint32(self.clear_flags)
        )
        set_flags = (
            None if self.set_flags is None else kuknos_xdr.Uint32(self.set_flags)
        )
        master_weight = (
            None
            if self.master_weight is None
            else kuknos_xdr.Uint32(self.master_weight)
        )
        low_threshold = (
            None
            if self.low_threshold is None
            else kuknos_xdr.Uint32(self.low_threshold)
        )
        med_threshold = (
            None
            if self.med_threshold is None
            else kuknos_xdr.Uint32(self.med_threshold)
        )
        high_threshold = (
            None
            if self.high_threshold is None
            else kuknos_xdr.Uint32(self.high_threshold)
        )
        signer = None if self.signer is None else self.signer.to_xdr_object()

        set_options_op = kuknos_xdr.SetOptionsOp(
            inflation_dest,
            clear_flags,
            set_flags,
            master_weight,
