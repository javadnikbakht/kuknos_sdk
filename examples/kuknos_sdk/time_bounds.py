...
class TimeBounds:
    ...
    **max_time** equal 0 means that it’s not set.
    ...
    <https://www.kuknos.org/developers/guides/concepts/transactions.html#time-bounds>`__
    ...
    :raises: :exc:`ValueError <kuknos_sdk.exceptions.ValueError>`: if ``max_time`` less than ``min_time``.
    """
    ...
    def to_xdr_object(self) -> kuknos_xdr.TimeBounds:
        """Returns the xdr object for this TimeBounds object.

        :return: XDR TimeBounds object
        """
        min_time = kuknos_xdr.TimePoint(kuknos_xdr.Uint64(self.min_time))
        max_time = kuknos_xdr.TimePoint(kuknos_xdr.Uint64(self.max_time))
        return kuknos_xdr.TimeBounds(min_time, max_time)

    @classmethod
    def from_xdr_object(cls, xdr_object: kuknos_xdr.TimeBounds) -> "TimeBounds":
