...
class FeeBumpTransaction:
    ...
    def __init__(
        ...
        if not isinstance(inner_transaction_envelope.transaction, Transaction):
            raise ValueError(
                "Invalid `inner_transaction`, it should be `kuknos_sdk.transaction.Transaction`."
            )
        ...
        self._fee_source_muxed: Optional[kuknos_xdr.MuxedAccount] = None
    ...
    def to_xdr_object(self) -> kuknos_xdr.FeeBumpTransaction:
        ...
        ext = kuknos_xdr.FeeBumpTransactionExt(0)
        inner_tx = kuknos_xdr.FeeBumpTransactionInnerTx(
            type=kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX,
            v1=self.inner_transaction_envelope.to_xdr_object().v1,
        )
        return kuknos_xdr.FeeBumpTransaction(
            fee_source=fee_source,
            fee=kuknos_xdr.Int64(fee),
            inner_tx=inner_tx,
            ext=ext,
        )
    ...
    @classmethod
    def from_xdr_object(
        cls, xdr_object: kuknos_xdr.FeeBumpTransaction, network_passphrase: str
    ) -> "FeeBumpTransaction":
        ...
        te = kuknos_xdr.TransactionEnvelope(
            type=kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX, v1=xdr_object.inner_tx.v1
        )
    ...
    def from_xdr(cls, xdr: str, network_passphrase: str) -> "FeeBumpTransaction":
        ...
        """
        xdr_object = kuknos_xdr.FeeBumpTransaction.from_xdr(xdr)
