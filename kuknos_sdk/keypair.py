...
from .sep.mnemonic import Language, KuknosMnemonic
...
class Keypair:
    ...
    verifying key for use with the Kuknos network.
    ...
    * :meth:`Keypair.from_public_key`
    ...
    def from_secret(cls, secret: str) -> "Keypair":
        ...
        :raise: :exc:`Ed25519SecretSeedInvalidError <kuknos_sdk.exceptions.Ed25519SecretSeedInvalidError>`:
    ...
    def from_public_key(cls, public_key: str) -> "Keypair":
        ...
        :raise: :exc:`Ed25519PublicKeyInvalidError <kuknos_sdk.exceptions.Ed25519PublicKeyInvalidError>`:
    ...
    def secret(self) -> str:
        ...
        :return: secret key
        :raise: :exc:`MissingEd25519SecretSeedError <kuknos_sdk.exceptions.MissingEd25519SecretSeedError>`
    ...
    def xdr_public_key(self) -> kuknos_xdr.PublicKey:
        """
        :return: xdr public key
        """
        return kuknos_xdr.PublicKey(
            kuknos_xdr.PublicKeyType.PUBLIC_KEY_TYPE_ED25519,
            kuknos_xdr.Uint256(bytes(self.verify_key)),
        )

    def xdr_account_id(self) -> kuknos_xdr.AccountID:
        return kuknos_xdr.AccountID(self.xdr_public_key())

    def xdr_muxed_account(self) -> kuknos_xdr.MuxedAccount:
        return kuknos_xdr.MuxedAccount(
            type=kuknos_xdr.CryptoKeyType.KEY_TYPE_ED25519,
            ed25519=kuknos_xdr.Uint256(bytes(self.verify_key)),
        )
    ...
    def verify(self, data: bytes, signature: bytes) -> None:
        ...
        :raise: :exc:`BadSignatureError <kuknos_sdk.exceptions.BadSignatureError>`:
    ...
    def generate_mnemonic_phrase(
        ...
        """
        mnemonic_phrase = KuknosMnemonic(language).generate(strength)
    ...
    def from_mnemonic_phrase(
        ...
        """
        raw_ed25519_seed = KuknosMnemonic(language).to_seed(
            mnemonic_phrase, passphrase, index
        )
        return cls.from_raw_ed25519_seed(raw_ed25519_seed)

    def sign_decorated(self, data) -> kuknos_xdr.DecoratedSignature:
        ...
        """
        hint = kuknos_xdr.SignatureHint(self.signature_hint())
        signature = kuknos_xdr.Signature(self.sign(data))
        return kuknos_xdr.DecoratedSignature(hint, signature)
