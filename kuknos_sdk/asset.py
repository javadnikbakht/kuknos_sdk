...
class Asset:
    ...
    corresponding issuer on the Kuknos network.

    For more information about the formats used for asset codes and how issuers
    work on Kuknos's network, see `Kuknos's guide on assets`_.

    :param code: The asset code, in the formats specified in `Kuknos's
        guide on assets`_.
    :param issuer: The account ID of the issuer. Note if the
        currency is the native currency (PMN (Paymons)), no issuer is necessary.
    :raises:
        | :exc:`AssetCodeInvalidError <kuknos_sdk.exceptions.AssetCodeInvalidError>`: if ``code`` is invalid.
        | :exc:`AssetIssuerInvalidError <kuknos_sdk.exceptions.AssetIssuerInvalidError>`: if ``issuer`` is not a valid ed25519 public key.

    .. _Kuknos's guide on assets:
        https://www.kuknos.org/developers/guides/concepts/assets.html
    """

    def __init__(self, code: str, issuer: Optional[str] = None) -> None:
        Asset.check_if_asset_code_is_valid(code)

        if code != "PMN" and issuer is None:
    ...
    def guess_asset_type(self) -> str:
        ...
        :return: The type of the asset.
        """
        if self.code == "PMN" and self.issuer is None:
            asset_type = "native"
    ...
    def to_xdr_object(self) -> kuknos_xdr.Asset:
        """Returns the xdr object for this asset.

        :return: XDR Asset object
        """
        if self.is_native():
            asset_type = kuknos_xdr.AssetType.ASSET_TYPE_NATIVE
            return kuknos_xdr.Asset(type=asset_type)
        else:
        ...
        if length <= 4:
            xdr_type = kuknos_xdr.AssetType.ASSET_TYPE_CREDIT_ALPHANUM4
            asset_code4 = kuknos_xdr.AssetCode4(asset_code)
            alpha_num4 = kuknos_xdr.AssetAlphaNum4(asset_code4, issuer)
            return kuknos_xdr.Asset(type=xdr_type, alpha_num4=alpha_num4)
        else:
            xdr_type = kuknos_xdr.AssetType.ASSET_TYPE_CREDIT_ALPHANUM12
            asset_code12 = kuknos_xdr.AssetCode12(asset_code)
            alpha_num12 = kuknos_xdr.AssetAlphaNum12(asset_code12, issuer)
            return kuknos_xdr.Asset(type=xdr_type, alpha_num12=alpha_num12)

    @classmethod
    def from_xdr_object(cls, xdr_object: kuknos_xdr.Asset) -> "Asset":
        ...
        """
        if xdr_object.type == kuknos_xdr.AssetType.ASSET_TYPE_NATIVE:
            return Asset.native()
        elif xdr_object.type == kuknos_xdr.AssetType.ASSET_TYPE_CREDIT_ALPHANUM4:
