...
__all__ = ["PayKuknosUri", "TransactionKuknosUri", "Replacement"]

STELLAR_SCHEME: str = "web+kuknos"


class KuknosUri(object, metaclass=abc.ABCMeta):
    ...
    def _signature_payload(self) -> bytes:
        ...
        # https://github.com/kuknos/kuknos-protocol/blob/master/ecosystem/sep-0007.md#request-signing
        sign_data = b"\0" * 35 + b"\4" + b"kuknos.sep.7 - URI Scheme" + data.encode()
        return sign_data
...
class PayKuknosUri(KuknosUri):
    """A request for a payment to be signed.

    See `SEP-0007 <https://github.com/kuknos/kuknos-protocol/blob/master/ecosystem/sep-0007.md#operation-pay>`_
    ...
    @classmethod
    def from_uri(cls, uri: str) -> "PayKuknosUri":
        """Parse Kuknos Pay URI and generate :class:`PayKuknosUri` object.

        :param uri: Kuknos Pay URI.
        :return: :class:`PayKuknosUri` object from uri.
        """
        parsed_uri = parse.urlparse(uri)
        if parsed_uri.scheme != STELLAR_SCHEME:
            raise ValueError(
                f"Kuknos URI scheme should be `{STELLAR_SCHEME}`, but got `{parsed_uri.scheme}`."
            )
        if parsed_uri.path != "pay":
            raise ValueError(
                f"Kuknos URI path should be `pay`, but got `{parsed_uri.path}`."
            )
    ...
    def __str__(self):
        return (
            f"<PayKuknosUri [destination={self.destination}, amount={self.amount}, "
...
class Replacement:
    def __init__(
        ...
        See `SEP-0007 <https://github.com/kuknos/kuknos-protocol/blob/master/ecosystem/sep-0007.md#operation-tx>`_
...
class TransactionKuknosUri(KuknosUri):
    ...
    def from_uri(
        ...
        """Parse Kuknos Transaction URI and generate :class:`TransactionKuknosUri` object.

        :param uri: Kuknos Transaction URI.
        ...
        :return: :class:`TransactionKuknosUri` object from uri.
        """
        parsed_uri = parse.urlparse(uri)
        if parsed_uri.scheme != STELLAR_SCHEME:
            raise ValueError(
        ...
        if parsed_uri.path != "tx":
            raise ValueError(
                f"Kuknos URI path should be `tx`, but got `{parsed_uri.path}`."
            )
        query = cls._parse_uri_query(parsed_uri.query)
    ...
    def __str__(self):
        return (
            f"<TransactionKuknosUri [xdr={self.transaction_envelope.to_xdr()}, replace={self.replace}, "
