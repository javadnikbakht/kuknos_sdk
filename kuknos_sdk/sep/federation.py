...
from .kuknos_toml import fetch_kuknos_toml
...
class FederationRecord:
    def __init__(self, account_id, kuknos_address, memo_type, memo):
        """The :class:`FederationRecord`, which represents record in federation server.

        :param account_id: Kuknos public key / account ID
        :param kuknos_address: Kuknos address
        ...
        self.kuknos_address: str = kuknos_address
        self.memo_type: Optional[str] = memo_type
        self.memo: Optional[str] = memo

    def __str__(self):
        return (
            f"<FederationRecord [account_id={self.account_id}, kuknos_address={self.kuknos_address}, "
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self.kuknos_address == other.kuknos_address
            and self.memo_type == other.memo_type
            and self.memo == other.memo
        )


def resolve_kuknos_address(
    kuknos_address: str,
    ...
    """Get the federation record if the user was found for a given Kuknos address.

    :param kuknos_address: address Kuknos address (ex. bob*kuknos.org).
    :param client: Http Client used to send the request.
    :param federation_url: The federation server URL (ex. `https://kuknos.org/federation`),
        if you don't set this value, we will try to get it from ``kuknos_address``.
    ...
    if isinstance(client, BaseAsyncClient):
        return __resolve_kuknos_address_async(
            kuknos_address, client, federation_url, use_http
        )
    elif isinstance(client, BaseSyncClient):
        return __resolve_kuknos_address_sync(
            kuknos_address, client, federation_url, use_http
        )
    else:
        raise TypeError(
...
def __resolve_kuknos_address_sync(
    kuknos_address: str,
    client: BaseSyncClient,
    federation_url: str = None,
    use_http: bool = False,
) -> FederationRecord:
    parts = split_kuknos_address(kuknos_address)
    domain = parts["domain"]
    if federation_url is None:
        federation_url = fetch_kuknos_toml(domain, use_http=use_http).get(  # type: ignore[union-attr]
    ...
    raw_resp = client.get(federation_url, {"type": "name", "q": kuknos_address})
    return __handle_raw_response(raw_resp, kuknos_address=kuknos_address)


async def __resolve_kuknos_address_async(
    kuknos_address: str,
    ...
    if federation_url is None:
        federation_url = (
            await fetch_kuknos_toml(domain, client=client, use_http=use_http)  # type: ignore[misc]
        ).get(FEDERATION_SERVER_KEY)
    if federation_url is None:
        raise FederationServerNotFoundError(
            f"Unable to find federation server at {domain}."
        )
    raw_resp = await client.get(federation_url, {"type": "name", "q": kuknos_address})
    return __handle_raw_response(raw_resp, kuknos_address=kuknos_address)
...
def __resolve_account_id_sync(
    ...
    if domain is not None:
        federation_url = fetch_kuknos_toml(domain, client, use_http).get(  # type: ignore[union-attr]
            FEDERATION_SERVER_KEY
        )
        if federation_url is None:
            raise FederationServerNotFoundError(
...
async def __resolve_account_id_async(
    ...
    if domain is not None:
        federation_url = (await fetch_kuknos_toml(domain, client, use_http)).get(  # type: ignore[misc]
            FEDERATION_SERVER_KEY
        )
        if federation_url is None:
            raise FederationServerNotFoundError(
...
def __handle_raw_response(
    raw_resp: Response, kuknos_address=None, account_id=None
) -> FederationRecord:
    if not 200 <= raw_resp.status_code < 300:
        raise BadFederationResponseError(raw_resp)
    data = raw_resp.json()
    account_id = account_id or data.get("account_id")
    kuknos_address = kuknos_address or data.get("kuknos_address")
    memo_type = data.get("memo_type")
    memo = data.get("memo")
    return FederationRecord(
        account_id=account_id,
        kuknos_address=kuknos_address,
        memo_type=memo_type,
        memo=memo,
    )


def split_kuknos_address(address: str) -> Dict[str, str]:
    parts = address.split(SEPARATOR)
    if len(parts) != 2:
        raise InvalidFederationAddress(
            "Address should be a valid address, such as `bob*kuknos.org`"
        )
    name, domain = parts
