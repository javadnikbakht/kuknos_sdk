"""
SEP: 0011
Title: Txrep: human-readable low-level representation of Kuknos transactions
...
def to_txrep(
    ...
    """Generate a human-readable format for Kuknos transactions.

    MuxAccount is currently not supported.

    Txrep is a human-readable representation of Kuknos transactions that functions like an assembly language for XDR.

    See `SEP-0011 <https://github.com/kuknos/kuknos-protocol/blob/master/ecosystem/sep-0011.md>`_


    :param transaction_envelope: Transaction envelope object.
    :return: A human-readable format for Kuknos transactions.
    """
...
def from_txrep(
    ...
    :param txrep: a human-readable format for Kuknos transactions.
...
def _get_signature(
    ...
    return kuknos_xdr.DecoratedSignature(
        kuknos_xdr.SignatureHint(hint), kuknos_xdr.Signature(signature)
    )


def _get_signatures(
    raw_data_map: Dict[str, str], prefix: str
) -> List[kuknos_xdr.DecoratedSignature]:
    signatures: List[kuknos_xdr.DecoratedSignature] = []
...
def _add_operation(
    ...
    if isinstance(operation, CreateAccount):
        ...
        # see https://github.com/javadnikbakht/py-kuknos-base/blob/master/.xdr/Kuknos-transaction.x#L282
...
def _add_signatures(
    signatures: List[kuknos_xdr.DecoratedSignature], prefix: str, lines: List[str]
) -> None:
...
def _add_signature(
    index: int, signature: kuknos_xdr.DecoratedSignature, prefix: str, lines: List[str]
) -> None:
