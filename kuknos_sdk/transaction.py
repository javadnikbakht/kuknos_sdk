...
class Transaction:
    ...
    <kuknos_sdk.transaction_envelope.TransactionEnvelope>` which is
    then signed before being sent over the network.

    For more information on Transactions in Kuknos, see `Kuknos's guide
    on transactions`_.

    .. _Kuknos's guide on transactions:
        https://www.kuknos.org/developers/guides/concepts/transactions.html
    ...
    :param fee: The fee amount for the transaction, which should equal
          ...
          operations in the transaction. See `Kuknos's latest documentation
          on fees
          <https://www.kuknos.org/developers/guides/concepts/fees.html#transaction-fee>`_
          for more information.
    :param operations: A list of operations objects (typically its
          subclasses as defined in :mod:`kuknos_sdk.operation.Operation`.
    ...
    :param memo: The memo being sent with the transaction, being
          ...
          :class:`Memo <kuknos_sdk.memo.Memo>` object.
    ...
    def __init__(
        ...
        self._source_muxed: Optional[kuknos_xdr.MuxedAccount] = None
    ...
    def to_xdr_object(
        self,
    ) -> Union[kuknos_xdr.Transaction, kuknos_xdr.TransactionV0]:
        ...
        fee = kuknos_xdr.Uint32(self.fee)
        sequence = kuknos_xdr.SequenceNumber(kuknos_xdr.Int64(self.sequence))
        ...
        if self.v1:
            ...
            ext = kuknos_xdr.TransactionExt(0)
            return kuknos_xdr.Transaction(
        ...
        ext_v0 = kuknos_xdr.TransactionV0Ext(0)
        return kuknos_xdr.TransactionV0(
    ...
    def from_xdr_object(
        cls,
        xdr_object: Union[kuknos_xdr.Transaction, kuknos_xdr.TransactionV0],
        v1: bool = False,
    ) -> "Transaction":
        """Create a new :class:`Transaction` from an XDR object.

        :param xdr_object: The XDR object that represents a transaction.
        :param v1: Temporary feature flag to allow alpha testing of Kuknos Protocol 13 transactions.
            We will remove this once all transactions are supposed to be v1.
            See `CAP-0015 <https://github.com/kuknos/kuknos-protocol/blob/master/core/cap-0015.md>`_
            for more information.

        :return: A new :class:`Transaction` object from the given XDR Transaction object.
        """
        if v1:
            assert isinstance(xdr_object, kuknos_xdr.Transaction)
            source = parse_ed25519_account_id_from_muxed_account_xdr_object(
                xdr_object.source_account
            )
        else:
            assert isinstance(xdr_object, kuknos_xdr.TransactionV0)
    ...
    def from_xdr(cls, xdr: str, v1: bool = False) -> "Transaction":
        ...
        """
        if v1:
            xdr_object = kuknos_xdr.Transaction.from_xdr(xdr)
            return cls.from_xdr_object(xdr_object, v1)
        xdr_object_v0 = kuknos_xdr.TransactionV0.from_xdr(xdr)
