...
class TransactionEnvelope(BaseTransactionEnvelope["TransactionEnvelope"]):
    ...
    class handles signing and conversion to and from XDR for usage on Kuknos's
    network.
    ...
    def __init__(
        ...
        signatures: List[kuknos_xdr.DecoratedSignature] = None,
    ) -> None:
    ...
    def signature_base(self) -> bytes:
        ...
        validators on the Kuknos Network will accept.
        ...
        if isinstance(self.transaction, Transaction) and not self.transaction.v1:
            ...
            tx.v1 = True
        packer = Packer()
        kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX.pack(packer)
        tx.to_xdr_object().pack(packer)
        return network_id + packer.get_buffer()

    def to_xdr_object(self) -> kuknos_xdr.TransactionEnvelope:
        ...
        if self.transaction.v1:
            assert isinstance(tx, kuknos_xdr.Transaction)
            te_type = kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX
            tx_v1_envelope = kuknos_xdr.TransactionV1Envelope(tx, self.signatures)
            return kuknos_xdr.TransactionEnvelope(type=te_type, v1=tx_v1_envelope)
        else:
            assert isinstance(tx, kuknos_xdr.TransactionV0)
            te_type = kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX_V0
            tx_v0_envelope = kuknos_xdr.TransactionV0Envelope(tx, self.signatures)
            return kuknos_xdr.TransactionEnvelope(type=te_type, v0=tx_v0_envelope)
    ...
    @classmethod
    def from_xdr_object(
        cls, xdr_object: kuknos_xdr.TransactionEnvelope, network_passphrase: str
    ) -> "TransactionEnvelope":
        ...
        if te_type == kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX_V0:
        ...
        elif te_type == kuknos_xdr.EnvelopeType.ENVELOPE_TYPE_TX:
