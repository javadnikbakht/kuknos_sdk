...
class BaseTransactionEnvelope(Generic[T]):
    def __init__(
        ...
        self.signatures: List[kuknos_xdr.DecoratedSignature] = signatures or []
    ...
    def sign(self, signer: Union[Keypair, str]) -> None:
        ...
        :param signer: The keypair or secret to use for signing this transaction
            envelope.
        :raise: :exc:`SignatureExistError <kuknos_sdk.exception.SignatureExistError>`:
    ...
    def sign_hashx(self, preimage: bytes) -> None:
        ...
        See Kuknos's documentation on `Multi-Sig
        <https://www.kuknos.org/developers/guides/concepts/multi-sig.html>`_
        ...
        hint = kuknos_xdr.SignatureHint(hash_preimage[-4:])
        sig = kuknos_xdr.DecoratedSignature(hint, kuknos_xdr.Signature(preimage))
        sig_dict = [signature.__dict__ for signature in self.signatures]
        if sig.__dict__ in sig_dict:
