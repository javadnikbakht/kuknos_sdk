...
class RevokeSponsorship(Operation):
    ...
    <https://developers.kuknos.org/docs/start/list-of-operations/#revoke-sponsorship>_`.
    ...
    _XDR_OPERATION_TYPE: kuknos_xdr.OperationType = kuknos_xdr.OperationType.REVOKE_SPONSORSHIP
    ...
    def _to_operation_body(self) -> kuknos_xdr.OperationBody:
        if self.revoke_sponsorship_type == RevokeSponsorshipType.ACCOUNT:
            ...
            account = kuknos_xdr.LedgerKeyAccount(
                Keypair.from_public_key(self.account_id).xdr_account_id()
            )
            ledger_key = kuknos_xdr.LedgerKey(
                kuknos_xdr.LedgerEntryType.ACCOUNT, account=account
            )
            revoke_sponsorship_op = kuknos_xdr.RevokeSponsorshipOp(
                kuknos_xdr.RevokeSponsorshipType.REVOKE_SPONSORSHIP_LEDGER_ENTRY,
                ledger_key=ledger_key,
            )
        elif self.revoke_sponsorship_type == RevokeSponsorshipType.TRUSTLINE:
            assert self.trustline is not None
            trust_line = kuknos_xdr.LedgerKeyTrustLine(
                Keypair.from_public_key(self.trustline.account_id).xdr_account_id(),
                self.trustline.asset.to_xdr_object(),
            )
            ledger_key = kuknos_xdr.LedgerKey(
                kuknos_xdr.LedgerEntryType.TRUSTLINE, trust_line=trust_line
            )
            revoke_sponsorship_op = kuknos_xdr.RevokeSponsorshipOp(
                kuknos_xdr.RevokeSponsorshipType.REVOKE_SPONSORSHIP_LEDGER_ENTRY,
                ledger_key=ledger_key,
            ...
            offer = kuknos_xdr.LedgerKeyOffer(
                Keypair.from_public_key(self.offer.seller_id).xdr_account_id(),
                kuknos_xdr.Int64(self.offer.offer_id),
            )
            ledger_key = kuknos_xdr.LedgerKey(
                kuknos_xdr.LedgerEntryType.OFFER, offer=offer
            )
            revoke_sponsorship_op = kuknos_xdr.RevokeSponsorshipOp(
                kuknos_xdr.RevokeSponsorshipType.REVOKE_SPONSORSHIP_LEDGER_ENTRY,
                ledger_key=ledger_key,
            ...
            data = kuknos_xdr.LedgerKeyData(
                Keypair.from_public_key(self.data.account_id).xdr_account_id(),
                kuknos_xdr.String64(self.data.data_name.encode()),
            )
            ledger_key = kuknos_xdr.LedgerKey(
                kuknos_xdr.LedgerEntryType.DATA, data=data
            )
            revoke_sponsorship_op = kuknos_xdr.RevokeSponsorshipOp(
                kuknos_xdr.RevokeSponsorshipType.REVOKE_SPONSORSHIP_LEDGER_ENTRY,
                ledger_key=ledger_key,
            ...
            balance_id = kuknos_xdr.ClaimableBalanceID.from_xdr_bytes(
                claimable_balance_bytes
            )
            claimable_balance = kuknos_xdr.LedgerKeyClaimableBalance(
                balance_id=balance_id
            )
            ledger_key = kuknos_xdr.LedgerKey(
                kuknos_xdr.LedgerEntryType.CLAIMABLE_BALANCE,
            ...
            signer_key = kuknos_xdr.RevokeSponsorshipOpSigner(
                Keypair.from_public_key(self.signer.account_id).xdr_account_id(),
                self.signer.signer_key.to_xdr_object(),
            )
            revoke_sponsorship_op = kuknos_xdr.RevokeSponsorshipOp(
                kuknos_xdr.RevokeSponsorshipType.REVOKE_SPONSORSHIP_SIGNER,
                signer=signer_key,
            )
        else:
            raise ValueError(
    ...
    @classmethod
    def from_xdr_object(cls, xdr_object: kuknos_xdr.Operation) -> "RevokeSponsorship":
        ...
        if op_type == kuknos_xdr.RevokeSponsorshipType.REVOKE_SPONSORSHIP_LEDGER_ENTRY:
            assert xdr_object.body.revoke_sponsorship_op is not None
            ...
            if ledger_key_type == kuknos_xdr.LedgerEntryType.ACCOUNT:
            ...
            elif ledger_key_type == kuknos_xdr.LedgerEntryType.TRUSTLINE:
            ...
            elif ledger_key_type == kuknos_xdr.LedgerEntryType.OFFER:
            ...
            elif ledger_key_type == kuknos_xdr.LedgerEntryType.DATA:
            ...
            elif ledger_key_type == kuknos_xdr.LedgerEntryType.CLAIMABLE_BALANCE:
        ...
        elif op_type == kuknos_xdr.RevokeSponsorshipType.REVOKE_SPONSORSHIP_SIGNER:
