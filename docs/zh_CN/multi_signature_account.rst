...
通过为账户启用 `多重签名 <https://www.kuknos.org/developers/learn/concepts/multi-sig.html>`_ ，你可以使这个账户创建的事务需要
经过多个公钥的签名才能生效。首先你需要为账户配置 **阈值(threshold)**，在 Kuknos 网络中，每个操作都有着自己的权限等级，这个等级分为高、中、低三等，
